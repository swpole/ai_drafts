...
rag_system.load_knowledge_base("my_knowledge.json")
...
with gr.Blocks(theme=gr.themes.Soft(), title="RAG Chat с VITS TTS") as demo:
    ...
    with gr.Row():
        with gr.Column(scale=1):
            ...
            model_selector = gr.Dropdown(
                choices=["gpt-oss:20b", "llama3:latest"],
                value="llama2",
    ...
    send_btn.click(
        ...
        outputs=[msg, chatbot, recognized_text, text_output, audio_output]
    )
    ...
    clear_btn.click(
        clear_chat,
        outputs=[chatbot, text_output, audio_output, recognized_text]
    )
    ...
    def add_documents(files):
        if files:
            ...
            return f"Добавлено {len(files)} документов в базу знаний"
...
if __name__ == "__main__":
    ...
    try:
        models = ollama.list()
        print("Доступные модели:", [model['model'] for model in models['models']])
    except Exception as e:
        print("Ошибка подключения к Ollama:", e)
        print("Убедитесь, что Ollama запущен: ollama serve")
    
    print("Загрузка модели VITS... Это может занять несколько минут.")
