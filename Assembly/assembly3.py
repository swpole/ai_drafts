...
# Количество параллельных аудиотреков
NUM_PARALLEL_TRACKS = 2
...
def insert_audio(audio_files, new_audio, selected_index, before: bool):
    """
    Вставляет new_audio либо перед (если before=True), либо после (if before=False)
    позиции selected_index в audio_files.
    Если selected_index is None или вне диапазона, просто добавляет в конец.
    """
...
def delete_audio(audio_files, selected_index):
    """
    Удаляет аудио по выбранному индексу.
    Если индекс невалиден, возвращает исходный список без изменений.
    """
    audio_list = list(audio_files) if audio_files else []
...
def create_video_with_transitions(gallery_images, duration_per_image, transition_duration, audio_tracks, output_filename="output_video.mp4"):
    """
    Создает видео из изображений с плавными переходами (кроссфейд) и объединенной звуковой дорожкой.
    """
            ...
            try:
                ...
                if track_audio_clips:
                    ...
                    if track_audio.duration < final_clip.duration:
                        ...
                        # Обрезаем аудио, если оно длиннее видео - исправлено на subclipped
...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        with gr.Column(scale=2):
            ...
            for i in range(NUM_PARALLEL_TRACKS):
                with gr.Group():
                    ...
                    audio_tracks.append(audio_gallery)
                    ...
                    def add_audio_before_handler(audio_files, upload, sel_idx, track_idx):
                        return insert_audio(audio_files, upload, sel_idx, before=True)

                    btn_audio_before.click(
                        add_audio_before_handler,
                        inputs=[audio_tracks[i], audio_uploaders[i], selected_audios[i]],
                        outputs=audio_tracks[i]
                    )

                    def add_audio_after_handler(audio_files, upload, sel_idx, track_idx):
                        return insert_audio(audio_files, upload, sel_idx, before=False)

                    btn_audio_after.click(
                        add_audio_after_handler,
                        inputs=[audio_tracks[i], audio_uploaders[i], selected_audios[i]],
                        outputs=audio_tracks[i]
                    )

                    def delete_audio_handler(audio_files, sel_idx, track_idx):
                        return delete_audio(audio_files, sel_idx)

                    btn_audio_delete.click(
                        delete_audio_handler,
                        inputs=[audio_tracks[i], selected_audios[i]],
                        outputs=audio_tracks[i]
                    )
        ...
        with gr.Column(scale=1):
            ...
            **Инструкция:**
            ...
            2. Добавьте аудиодорожки в {NUM_PARALLEL_TRACKS} параллельных трека
    ...
    btn_before.click(
        add_before_handler,
        inputs=[gallery, new_img_upload, new_img_url, selected_image],
        outputs=gallery
    )
    ...
    def create_video_handler(gallery_images, duration, transition, *audio_track_data):
        try:
            ...
            filename = audio_track_data[-1]
            
            video_path = create_video_with_transitions(gallery_images, duration, transition, all_audio_tracks, filename)
            return video_path
        except Exception as e:
    ...
    for i in range(NUM_PARALLEL_TRACKS):
        input_components.append(audio_tracks[i])
