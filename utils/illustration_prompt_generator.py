...
class IllustrationPromptGenerator:
    def __init__(self):
        self.base_prompt = """Ты — эксперт по визуализации текста и созданию иллюстрационных промптов.
...
Итог должен быть списком промптов для генерации иллюстраций, которые можно подавать напрямую в графическую модель."""
        ...
        self.illustration_interface = LLMInterface(
            ...
            prompt_default=self.base_prompt.format(style="реализм"),
    ...
    def update_prompt(self, style):
        """Обновление промпта при смене стиля"""
        return self.base_prompt.format(style=style)
    ...
    def run_generation(self, model_name, prompt, input_text):
    ...
    def demo(self):
        ...
        with gr.Blocks(title=self.illustration_interface.title) as demo:
